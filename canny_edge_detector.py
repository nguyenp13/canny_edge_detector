...
Canny edge detection is a technique for reducing the amount of content needed to be processed by computer vision systems. 
...
def usage(): 
    # Sample Usage: python canny_edge_detector.py input.png ./results -denoising_sigma 1.5 -low_threshold 25 -high_threshold 75
    print >> sys.stderr, ''
    print >> sys.stderr, 'Usage: python '+__file__+' input_image out_dir <options>'
    print >> sys.stderr, ''
    print >> sys.stderr, 'Sample Usage: python '+__file__+' input.png ./results -denoising_sigma 9 -low_threshold 35 -high_threshold 75'
    print >> sys.stderr, ''
    print >> sys.stderr, 'The Canny Edge Detector is an algorithm used for extracting useful structural information from an image. 
    print >> sys.stderr, ''
    ...
    print >> sys.stderr, ''
    print >> sys.stderr, '    -low_threshold <float>'
    print >> sys.stderr, '        The low threshold cut off used in the ysteresis thresholding. The default value is 25. '
    print >> sys.stderr, ''
    print >> sys.stderr, '    -high_threshold <float>'
    print >> sys.stderr, '        The high threshold cut off used in the ysteresis thresholding. The default value is 75. '
    print >> sys.stderr, ''
    exit(1)
...
def main():
    ...
    low_threshold = float(get_command_line_param_val_default_value(sys.argv,'-low_threshold',25)) # the lower threshold to be used for hysteresis thresholding
    high_threshold = float(get_command_line_param_val_default_value(sys.argv,'-high_threshold',75)) # the higher threshold to be used for hysteresis thresholding 
    makedirs(out_dir)
